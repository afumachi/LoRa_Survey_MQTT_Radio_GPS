...
class VisualizadorGPS:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Monitor de Enlace LoRa - GPS & Distância")
        ...
        self.criar_widgets()
        self.carregar_dados_gateway_iniciais()
        self.atualizar_dados_no()

    def criar_widgets(self):
        # Frame Gateway Fixo (Editável)
        frame_gw = ttk.LabelFrame(
            self.root, text=" Configuração do Gateway Fixo ", padding=10
        )
        frame_gw.pack(fill="x", padx=15, pady=8)
        ...
        # Frame Nó Sensor (Apenas Leitura)
        frame_node = ttk.LabelFrame(
            self.root, text=" Nó Sensor (Calculado) ", padding=10
        )
        frame_node.pack(fill="x", padx=15, pady=8)
        ...
        # Frame Distância
        frame_dist = ttk.Frame(self.root, padding=5)
        frame_dist.pack(fill="x", padx=15, pady=5)
    ...
    def carregar_dados_gateway_iniciais(self):
        """Preenche as caixas de texto com os dados do arquivo gateway_gps.txt."""
        gw_lat, gw_lon, gw_alt = ler_gateway_gps()
    ...
    def salvar_gateway(self):
        """Valida as coordenadas e salva no arquivo gateway_gps.txt."""
        try:
            lat = float(self.entry_gw_lat.get().strip().replace(",", "."))
            lon = float(self.entry_gw_lon.get().strip().replace(",", "."))
            alt = float(self.entry_gw_alt.get().strip().replace(",", "."))
            ...
            messagebox.showerror(
                "Erro de Validação",
                "Insira valores numéricos válidos para a Latitude, Longitude e Altitude.",
            )
            return
    ...
    def atualizar_dados_no(self):
        """Atualiza periodicamente apenas a leitura do Nó Sensor (gps.tmp)."""
        dados_node = ler_ultimo_gps()
        if dados_node:
            self.node_lat, self.node_lon, self.node_alt = (
                dados_node[0],
        ...
        self.root.after(2000, self.atualizar_dados_no)

    def recarregar_tudo(self):
        """Recarrega tanto os dados do Gateway quanto os do Nó Sensor."""
        self.carregar_dados_gateway_iniciais()
        self.atualizar_dados_no()

    def abrir_maps(self):
        """Abre a rota entre as coordenadas atuais dos campos do Gateway e o Nó Sensor."""
        try:
            gw_lat = float(self.entry_gw_lat.get().strip().replace(",", "."))
            gw_lon = float(self.entry_gw_lon.get().strip().replace(",", "."))
        except ValueError:
...
# Definição do diretório de parâmetros conforme especificado
dir_nivel4 = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "../3_N4_Armazenamento/Parametros/",
)
...
def ler_gateway_gps():
    ...
    Suporta formato 'GW_LAT = -23...' ou valores separados por vírgula/linha.
    """
    gw_lat, gw_lon, gw_alt = GW_LAT_DEFAULT, GW_LON_DEFAULT, GW_ALT_DEFAULT
    ...
    if os.path.exists(arquivo_gw_gps):
        try:
            ...
            # Tenta decodificar se estiver no formato de atribuição (GW_LAT = -23...)
            valores_dict = {}
            for linha in conteudo.splitlines():
                if "=" in linha and not linha.strip().startswith("#"):
                    chave, val = linha.split("=", 1)
            ...
            # Se não for chave=valor, extrai os números diretamente (separados por vírgula ou linha)
            floats = [
                float(x) for x in re.findall(r"[-+]?\d*\.\d+|\d+", conteudo)
            ]
            if len(floats) >= 3:
...
class VisualizadorGPS:
    ...
    def __init__(self, root):
        ...
        self.gw_lat = GW_LAT_DEFAULT
        self.gw_lon = GW_LON_DEFAULT
        self.gw_alt = GW_ALT_DEFAULT
    ...
    def criar_widgets(self):
        # Frame Gateway Fixo
        frame_gw = ttk.LabelFrame(self.root, text=" Gateway Fixo ", padding=10)
        frame_gw.pack(fill="x", padx=15, pady=8)

        self.lbl_gw_lat = ttk.Label(frame_gw, text="Latitude: --")
        self.lbl_gw_lat.pack(anchor="w")
        self.lbl_gw_lon = ttk.Label(frame_gw, text="Longitude: --")
        self.lbl_gw_lon.pack(anchor="w")
        self.lbl_gw_alt = ttk.Label(frame_gw, text="Altitude: --")
        self.lbl_gw_alt.pack(anchor="w")
        ...
        btn_refresh = ttk.Button(
            frame_botoes, text="🔄 Atualizar", command=self.atualizar_dados
        )
        btn_refresh.pack(side="right", expand=True, fill="x", padx=5)

    def atualizar_dados(self):
        # Atualiza dados do Gateway via arquivo gateway_gps.txt
        self.gw_lat, self.gw_lon, self.gw_alt = ler_gateway_gps()
        self.lbl_gw_lat.config(text=f"Latitude: {self.gw_lat:.6f}°")
        self.lbl_gw_lon.config(text=f"Longitude: {self.gw_lon:.6f}°")
        self.lbl_gw_alt.config(text=f"Altitude: {self.gw_alt:.1f} m")

        # Atualiza dados do Nó Sensor via arquivo gps.tmp
        dados_node = ler_ultimo_gps()
        if dados_node:
            self.node_lat, self.node_lon, self.node_alt = (
                dados_node[0],
        ...
        self.root.after(2000, self.atualizar_dados)

    def abrir_maps(self):
        if self.node_lat is None or self.node_lon is None:
            messagebox.showwarning(
                "Aviso",
                "Ainda não existem coordenadas do Nó Sensor no arquivo gps.tmp.",
            )
            return

        url = f"https://www.google.com/maps/dir/?api=1&origin={self.gw_lat},{self.gw_lon}&destination={self.node_lat},{self.node_lon}"
        webbrowser.open(url)
