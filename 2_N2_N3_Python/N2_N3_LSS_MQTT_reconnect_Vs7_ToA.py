...
def on_connect(client, userdata, flags, reason_code, properties):
    ...
    if reason_code == 0:
        ...
        print(f"[MQTT] Falha na conexão. Código: {reason_code}")
        estado_mqtt = 0
        # REMOVIDO: client.reconnect() -> O loop_start cuida das tentativas!
...
def on_disconnect(client, userdata, flags, reason_code, properties):
    ...
    if reason_code != 0:
        ...
        print("[MQTT] Desconectado graciosamente.")
    # REMOVIDO: client.reconnect() -> Isso evitava que a thread do Paho morresse com gaierror!
...
# Ajuste do tempo de retentativa entre conexões
# Tenta reconectar a cada 1s no início, aumentando gradativamente até 15s de pausa
client.reconnect_delay_set(min_delay=1, max_delay=15)

print("[MQTT] Conectando ao broker MQTT - porta 1883...")

# Usamos connect_async para não travar a inicialização do programa se estiver sem internet no startup
client.connect_async(BROKER, PORTA_MQTT, keepalive=15)

# Inicia a thread de fundo que irá gerenciar socket, reconexões e callbacks
client.loop_start() 
