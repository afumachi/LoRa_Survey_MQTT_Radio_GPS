...
'''
# Bibliotecas MQTT anteriores a implemetação Gemini para reconnect MQTT Broker
...
def on_message(client, userdata, msg):
    ...
    if len(payload) >= Tamanho_pacote:
        ...
        Pacote_UL_status.set()

def on_disconnect(client, userdata, flags, reason_code, properties):
    global estado_mqtt
    estado_mqtt = 0
    if reason_code != 0:
        print(f"[MQTT] Desconectado inesperadamente (rc={reason_code}). Aguardando reconexão automática do loop...")
    else:
        print("[MQTT] Desconectado graciosamente.")
...
# Chamada corrigida especificando os parâmetros explicitamente
client.connect_async(host=BROKER, port=PORTA_MQTT, keepalive=15)

client.loop_start() 

# Aguarda estado_mqtt virar 1 ou timeout de inicialização
timeout_conexao = 5
inicio = time.time()
while estado_mqtt == 0 and (time.time() - inicio) < timeout_conexao:
    time.sleep(0.2)
   ...
   if os.path.exists(caminho_csv_end_devices):
       try:
           ...
           if not df_devices.empty:
               ...
               # Obtém o endereço de rede do sensor na linha atual
               endereco_hex_ou_str = str(df_devices.iloc[indice_dispositivo_atual]['endereco_rede']).strip()
               
               # Converte o endereço (seja '0x01' em hex ou inteiro) para int
               if endereco_hex_ou_str.startswith("0x") or endereco_hex_ou_str.startswith("0X"):
                   ID_sensor = int(endereco_hex_ou_str, 16)
               else:
                   ID_sensor = int(endereco_hex_ou_str)
               
               print(f"[LSS] Enviando Pacote DL para o Nó Sensor [{indice_dispositivo_atual + 1}/{len(df_devices)}]: Endereço 0x{ID_sensor:02X} ({ID_sensor})")
               
               # Incremente o índice para a próxima execução da função downlink
               indice_dispositivo_atual += 1
           else:
               print("[AVISO] O arquivo end_devices_net_par.csv está vazio. Mantendo ID_sensor anterior.")
       except Exception as e:
           print(f"[ERRO] Falha ao ler {caminho_csv_end_devices}: {e}")
   else:
       print(f"[AVISO] Arquivo {caminho_csv_end_devices} não encontrado. Mantendo ID_sensor = {ID_sensor}")
