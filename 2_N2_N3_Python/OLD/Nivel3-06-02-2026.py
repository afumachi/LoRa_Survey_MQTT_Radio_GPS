...
valor_init_PW = 20 # TX Power = 1 a 17 ?
...
valor_run_PW = 20 # TX Power = 1 a 17 ?
...
valor_new_PW = 20 # TX Power = 1 a 17 ?
...
def calculate_lora_toa(payload, valor_run_SF, valor_bandwidth, cr, n_preambulo=8, header_impl=False, crc_on=True, low_dr_opt=None):
    # 1. Parâmetros de tempo básicos
    BANDWIDTH_hz = valor_bandwidth * 1000
    tempo_simbolo = (2**valor_run_SF) / BANDWIDTH_hz
    ...
    n_payload_simbolo = 8 + max(math.ceil(term) * (cr_val + 4), 0)
    
    tempo_payload = n_payload_simbolo * tempo_simbolo
    
    # Retorna ToA em ms e bitrate em bps
    ToA_ms = (tempo_preambulo + tempo_payload) * 1000
...
def downlink():
   ...
   Pacote_DL[7] = cmd_new_config  # Envio pelo Byte [11] do Pacote_DL o comando de alterar config. Rádio LoRa

   # Camada MAC
   Pacote_DL[6]= valor_tempo
...
def uplink():
   ...
   if(len(Pacote_UL)==tamanho_do_pacote): #52
      ...
      confirm_new_config = Pacote_UL[7] # recebe info de confirmação da nova configuração de rádio LoRa    
...
while True:
   ...
   if (valor_bandwidth == 125):
       valor_new_BW = 1
   elif (valor_bandwidth == 250):
       valor_new_BW = 2
   elif (valor_bandwidth == 500):  
       valor_new_BW = 3
