...
global tamanho_do_pacote, taxa_canal_teorica, taxa_canal_calculada, bitrate, medida_atual
 # definições de teste: configurações importantes para a bateria de testes extraídas do arquivo de parâmetros

tempo_inicio_espera = None
esperando_pacote = False
TIMEOUT_SERIAL = 8  # segundos
...
#========== UPLINK ==================
def uplink():
   global perda_geral, rssi_DL, rssi_UL, contador_UL, ultimo_pacote_DL, air_quality_indicator, Pacote_UL
   global luminosidade, confirma_mudar_radio
   global tempo_inicio_espera, esperando_pacote
   ...
   # INICIO DA ESPERA
   # -----------------------------
   if not esperando_pacote:
       tempo_inicio_espera = time.monotonic()
       esperando_pacote = True
   
   print("tempo de espera", tempo_inicio_espera)
   # -----------------------------
   ...
   if (ser is not None):
       ...
       if(ser.in_waiting >= tamanho_do_pacote):
   ...
   if ser.in_waiting >= tamanho_do_pacote:
        ...
        if len(Pacote_UL_bytes) == tamanho_do_pacote:
            ...
            Pacote_UL = list(Pacote_UL_bytes)
   ...
   if(len(Pacote_UL)>=tamanho_do_pacote): #52
      ...
      if(Pacote_UL[8]== 0 and Pacote_UL[10] ==1):
         ...
         # 1. Limpa o que estiver sobrando antes de iniciar uma nova leitura importante
   ...
   if esperando_pacote:
       ...
       if tempo_atual - tempo_inicio_espera >= TIMEOUT_SERIAL:
           
           perda_geral += 1
           print("### TIMEOUT - Pacote não recebido:", perda_geral)

           # Reset para esperar novo pacote
           esperando_pacote = False
           tempo_inicio_espera = time.monotonic()
...
def gravaLOG_Gerencia():
     ...
     print(medida_atual, ";", rssi_DL, ";", round(psr_geral, 2), ";", round(psr_geral, 2), ";", rssi_UL, ";", rssi_max_dl, ";", rssi_min_dl, ";", rssi_max_ul, ";", rssi_min_ul, ";", valor_atual_spreadingfactor, ";", valor_atual_bandwidth, ";", valor_atual_codingrate, ";", valor_atual_potencia_radio, ";", round(taxa_canal_teorica, 2), ";", round(taxa_canal_calculada, 2), file=gerencia, sep='')
     gerencia.close()
     
     # 2. Grava no arquivo de LOG definitivo
     log_def = open(arquivo_LOG_gerencia, 'a')
     print(strftime("%d/%m/%Y %H:%M:%S"), ";", medida_atual, ";", rssi_DL, ";", rssi_UL, ";", perda_geral, ";", round(psr_geral, 2), ";", rssi_max_dl, ";", rssi_min_dl, ";", rssi_max_ul, ";", rssi_min_ul, ";", valor_atual_spreadingfactor, ";", valor_atual_bandwidth, ";", valor_atual_codingrate, ";", valor_atual_potencia_radio, ";", taxa_canal_teorica, ";", taxa_canal_calculada, file=log_def, sep='')
     log_def.close()
...
def calculaTaxaCanal():
    global taxa_canal_teorica, taxa_canal_calculada, psr_geral, valor_novo_bandwidth, valor_atual_spreadingfactor, valor_atual_codingrate
    ...
    taxa_canal_teorica = 1000*(valor_atual_spreadingfactor*((valor_novo_bandwidth*1000)/(2**valor_atual_spreadingfactor))*(4/(4+valor_CR)))/1000
...
while True:
   ...
   if (condicao_start == 1):
      ...
      if (medida_atual == 0): # and (start_teste_site_suvey == 1)):
         ...
         print ('Time stamp;Contador;RSSI_DL;RSSI_UL;Perdas;PSR;Max_DL;Min_DL;Max_UL;Min_UL;valor_atual_spreadingfactor;valor_atual_bandwidth;valor_atual_codingrate;valor_atual_potencia_radio;taxa_canal_teorica;taxa_canal_calculada', file=f)
